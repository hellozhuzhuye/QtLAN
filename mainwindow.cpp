...
void MainWindow::onLine(QString name, QString time)
{
    ...
    if (notExist) {
        ...
        QTableWidgetItem* uname = new QTableWidgetItem(name);
        QTableWidgetItem* ip = new QTableWidgetItem(getLocHostIp());
        QTableWidgetItem* host = new QTableWidgetItem(QHostInfo::localHostName());

        ui->userListTableWidget->insertRow(rowCount);
        ui->userListTableWidget->setItem(rowCount, 0, uname);
        ui->userListTableWidget->setItem(rowCount, 1, ip);
        ui->userListTableWidget->setItem(rowCount, 2, host);
...
{
    if (myname == rmtname) {
        int result = QMessageBox::information(this, tr("收到文件"), tr("好友 %1 向您发送文件：\r\n%2，是否接收？").arg(name).arg(filename), QMessageBox::Yes, QMessageBox::No);
        if (result == QMessageBox::Yes) {
            QString fname = QFileDialog::getSaveFileName(0, tr("保存 "), filename);
            if (!fname.isEmpty()) {
                FileCntDialog* fcnt = new FileCntDialog(this);
                fcnt->setFileInfo(fname);
